...
| 19 | Source registry + free-tier ETL layer | Assistant / CodeMike | todo | Build the `destination_sources_v1.csv` file the v2 database strategy §Core Tables 4 named, plus a `src/codemike/data/sources/` ETL layer pulling **free-tier-only** data per strategy doc §19.3: (a) OpenFlights routes + OurAirports lookup for origin-fit derivation; (b) OpenStreetMap Overpass API for hospital / medical-facility POI extraction (medical access derivation); (c) IMD historical climate + ECMWF / NOAA reanalysis for season-window derivation; (d) MEA / state-tourism portal scrapes (with provenance) for permit / visa status. Each source gets a per-row citation. Architecture decision blocked by §19.6 open question (refresh granularity — Option b two-tier vs Option c all-live). |
| 20 | v2 no-assumption enrichment strategy doc | Assistant / CodeMike | todo | New document at `datasets/reference/destination_master_enrichment_strategy_v2.md`. Supersedes §§0–17 of the v1 strategy. Specifies the no-assumption pipeline per §19.1 (no workspace-judgement values; every field source-backed or `unknown_pending_research`); the source ladder per §19.3 (Tier 1 preferred → Tier 4 acceptable; paid out of scope); the live-refresh discipline per §19.4; the unknown discipline per §19.5; the chosen refresh-granularity architecture per §19.6 resolution. Each spec field gets a documented source plan + a fallback `unknown_pending_research` rule. **Blocked on §19.6 architecture decision** (two-tier vs all-live). |
| 21 | v2 source-backed enrichment service | Assistant / CodeMike | todo | Implements the v2 strategy (P20). New module at `src/codemike/data/destination_master_enrichment_v2/` (package, not single file — pipeline is bigger). The v1.0 script (`destination_master_enrichment_v1.py`) is renamed to `destination_master_enrichment_v0_heuristic.py` and preserved as the diff-baseline per §19.2. The v2 service produces a refreshed enriched layer (architecture per §19.6) with per-row + per-field source citations + `manual_research_needed` flags. Blocked on P19 + P20. |
...
Next design step now: resolve the §19.6 open architecture question — refresh-on-query granularity (Option b two-tier vs Option c all-live). This decision unblocks P20 (v2 strategy doc), which unblocks P19 (source registry scope) and P21 (v2 service architecture).
