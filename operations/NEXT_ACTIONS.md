...
| 19 | Source registry + free-tier ETL layer (stable + live split) | Assistant / CodeMike | **next** | Per §19.6 two-tier architecture: build (a) the **stable-derived layer** as a batch ETL producing `destinations_master_v2_derived.csv` from free-tier sources — route distance from CCU/IXR via OpenFlights+OurAirports, hospital POI distances via OSM Overpass, baseline climatology via IMD/ECMWF, altitude profile via SRTM/free DEM; (b) the **live-volatile fetch layer** as a per-query service interface for current flight availability, current weather, current permit/visa, current park closure — never cached. Also build `destination_sources_v1.csv` (the source registry the v2 database strategy §Core Tables 4 named). Each output field carries a `source_id` + `fetched_at`. Each source's free-tier rate-limit + endpoint + auth method documented. |
| 20 | v2 no-assumption enrichment strategy doc | Assistant / CodeMike | **done** | Landed 2026-06-01 at `datasets/reference/destination_master_enrichment_strategy_v2.md`. Supersedes §§0–17 of the v1 strategy (v1 §18–§19 stay binding). Specifies all five §19 disciplines + the §4 two-tier architecture finalised. §7 is the per-field catalogue (source / layer / derivation / unknown fallback) over the `charter/TOOLING.md` pre-approved source set only. **One flagged design change (§6, needs review)**: composite bands (suitability / fatigue / planning-complexity) moved out of enrichment to the scoring layer (P6) per §19.1's ban on stacked-heuristic composites; v2 enrichment stores atomic source-backed facts only. §7 source assignments are **candidate until P19's coverage matrix locks them**. Next deliverable: P19. |
...
**Update 2026-06-01 (P20 ship)**: v2 strategy doc landed at `datasets/reference/destination_master_enrichment_strategy_v2.md`. Operationalises §19 into a per-field source/layer/derivation/unknown catalogue (§7) over the pre-approved free-tier source set. Two items now stand: (1) **§6 atomic-vs-composite decision needs review** — v2 moves composite bands to the P6 scoring layer per §19.1's ban on stacked-heuristic composites; confirm before P21 implements; (2) **P19 is the immediate next deliverable** — the source registry + coverage matrix gates §7's source assignments from *candidate* to *locked*.
