...
| Calibration-cycle discipline (spec → ship → calibration → policy revision) | 2 | Four-step research-methodology pattern at workspace scale: (1) author a spec rigorous enough to ship; (2) ship the spec against real data; (3) capture the calibration findings the run surfaces (silent tweaks rejected — findings land as durable text); (4) revise policy when the calibration findings warrant a category-level rethink rather than a parameter tweak. Treats artifacts produced before the policy revision as the *heuristic baseline* — not deleted, not rolled back, retained as diff target + research-priority signal. | Demonstrated end-to-end in the destination-enrichment workstream 2026-05-18: strategy doc v1 spec (P5) → E1 v1.0 ship (P15) → strategy doc §18 calibration amendments → §19 no-assumption + live-data policy revision → §19.6 two-tier architecture decision. Four commits, one branch, durable artifact at each step (`datasets/reference/destination_master_enrichment_strategy.md` §18 + §19 + §19.6; `src/codemike/data/destination_master_enrichment_v1.py` preserved as v0 heuristic baseline per §19.2). | Re-run the cycle on P20 (v2 source-backed strategy doc) — second worked example will promote to maturity 3 (reusable pattern). Capability card under `capabilities/` after the third worked example accumulates. |
